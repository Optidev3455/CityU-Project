...
"""
Use v8x for precision
Use v8n for speed
"""
Model = YOLO("yolov8n.pt").cuda()

# Global dictionary to store counts of detections with timestamps
total_boxes_drawn = 0
cameraCount = 1
frames_counted = 0
highest_count = 0
...
def DetectAndCountTouch(Results, Frame, TargetObjects, TouchThreshold):
    ...
    # Initialize touch count for this frame
    touch_count = 0  
...
def generate_heatmap(Frame):
    ...
    # Create an empty image for the heatmap with the same shape as the camera frame
    heatmap_image = np.zeros((480, 640), dtype=np.float32)  

    # Add points to the heatmap with larger dot sizes by incrementing more pixels around each point
    for x, y in touch_coordinates:
        if 0 <= x < 640 and 0 <= y < 480:  
            heatmap_image[y-10:y+10, x-10:x+10] += 10   # Increase dot size and intensity

    # Normalize and apply Gaussian Blur to smooth out the heatmap
    heatmap_image = cv2.GaussianBlur(heatmap_image, (21, 21), 0)
    heatmap_image = cv2.normalize(heatmap_image, None, alpha=0, beta=255,
                                   norm_type=cv2.NORM_MINMAX)

    # Convert to color map with transparency effect applied later on frame overlaying.
...
def generate_frames():
    while True:
        ...
        TargetObjects = ["cell phone",  'box']
        TouchThreshold = 1250
        
        DetectAndCountTouch(Results, Frame, TargetObjects, TouchThreshold)

        PutFPSText(Frame)

        # Encode frame as JPEG without overlaying heatmap for video feed.
...
def generate_heatmap_frames():
    while True:
       ...
       # Encode image as JPEG for serving via HTTP.
...
def get_detection_counts():
    ...
    if elapsed_time >= 10:
        response_data = {
            "highestCount": math.ceil(highest_count),
...
# Release resources after stopping the server.
Camera.release()
